...
public class GameMap {
    ...
    private Map<Integer, TextureRegion> mapTextures = new HashMap<>();
    ...
    public GameMap(String filePath) {
        ...
        mapTextures.put(0, tiles[0][0]); // Wall (horizontal)
        mapTextures.put(1, tiles[2][1]); // Entry point
        mapTextures.put(2, tiles[6][0]); // Exit
        mapTextures.put(3, things[4][6]); // Trap (static obstacle)
        mapTextures.put(4, mobs[4][6]); // Enemy (dynamic obstacle)
        mapTextures.put(5, things[0][6]); // Key
        mapTextures.put(6, tiles[0][1]); // Wall (vertical)
        mapTextures.put(7, tiles[0][2]); // Wall (L-section)
        mapTextures.put(8, tiles[0][3]); // Wall (T-section)
        mapTextures.put(9, tiles[1][6]); // Floor
        mapTextures.put(10, tiles[8][1]); // Outside
        ...
        for (int x = 0; x < mapWidth; x++) {
            for (int y = 0; y < mapHeight; y++) {
                ...
                if (!gameMapMap.containsKey(currentCoordinateTuple)) {
                    cell.setTile(new StaticTiledMapTile(mapTextures.get(9))); // Floor
                } else {
                    cell.setTile(new StaticTiledMapTile(mapTextures.get(gameMapMap.get(currentCoordinateTuple)))); // Set the specified tile image
                    ...
                    if (gameMapMap.get(currentCoordinateTuple) == 0) {
                        ...
                        if (!southWall) {
                            if(northWall){
                                if(eastWall){
                                    cell.setTile(new StaticTiledMapTile(mapTextures.get(7))); // Set wall as L-segment
                                } else {
                                    TextureRegion flippedHorizontally = new TextureRegion(mapTextures.get(7));
                                    flippedHorizontally.flip(true, false);
                                    cell.setTile(new StaticTiledMapTile(flippedHorizontally)); // Set wall as L-segment (flipped on y-axis)
                                }
                            }
                        } else {
                            cell.setTile(new StaticTiledMapTile(mapTextures.get(6))); // Set wall as a T-segment
                            if (northWall) {
                                cell.setTile(new StaticTiledMapTile(mapTextures.get(6))); // Set wall as vertical-segment
                            } else {
                                cell.setTile(new StaticTiledMapTile(mapTextures.get(8))); // Set wall as a T-segment
                            }
                        }
